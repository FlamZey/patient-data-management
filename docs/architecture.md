...
**The fallback path isn't full decrypt-then-filter either.** `Patient` has 35 PHI-bearing columns, but `list_patients`' fallback only ever filters or sorts on 4 (`first_name`, `last_name`, `date_of_birth`, `gender`). `_decrypt_core_fields` decrypts just those 4 for every candidate row; the other 31 optional fields (`_decrypt_patient`, full `PatientRead`) are only decrypted for the page actually returned, after filtering/sorting/pagination has already thrown the rest away. A candidate set of, say, 2,000 rows that gets filtered down to a 25-row page pays for roughly 8,000 field decryptions instead of 62,000.
