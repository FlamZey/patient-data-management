...
- **Fallback path** — a request that filters or sorts by a PHI field decrypts the SQL-narrowed candidate set (any `patient_code` filter already applied) and finishes filtering/sorting/pagination in application code — and even then, only the 4 fields (`first_name`, `last_name`, `date_of_birth`, `gender`) that filtering/sorting can actually use are decrypted for every candidate row; the other 31 optional PHI fields are decrypted only for the page finally returned.
